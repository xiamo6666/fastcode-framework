...
import com.ssos.flowable.modeler.model.decisiontable.DecisionTableRepresentation;
...
public class FlowableDecisionTableService extends BaseFlowableModelService {
    ...
    protected void exportDecisionTable(HttpServletResponse response, AbstractModel decisionTableModel) {
        ...
        try {
            ...
            throw new RuntimeException("Could not export decision table model");
        }
    }
    ...
    public ModelRepresentation importDecisionTable(HttpServletRequest request, MultipartFile file) {
        ...
        if (fileName != null && (fileName.endsWith(".dmn") || fileName.endsWith(".xml"))) {
            try {
                ...
                Model model = modelService.createModel(modelRepresentation, editorJsonNode.toString(),null);
                return new ModelRepresentation(model);

            } catch (Exception e) {
                LOGGER.error("Could not import decision table model", e);
                throw new RuntimeException("Could not import decision table model");
            }
        } else {
            throw new RuntimeException("Invalid file name, only .dmn or .xml files are supported not " + fileName);
        }
    ...
    protected DecisionTableRepresentation createDecisionTableRepresentation(AbstractModel model) {
        ...
        try {
            ...
            throw new RuntimeException("Could not deserialize decision table definition");
        }
    ...
    public DecisionTableRepresentation saveDecisionTable(String decisionTableId, DecisionTableSaveRepresentation saveRepresentation) {
        ...
        if (modelKeyInfo.isKeyAlreadyExists()) {
            throw new RuntimeException("Provided model key already exists: " + decisionKey);
        }
        ...
        try {
            ...
            throw new RuntimeException("Decision table could not be saved " + decisionTableId);
        }
