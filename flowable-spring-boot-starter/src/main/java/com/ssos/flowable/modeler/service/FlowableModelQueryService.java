    ...
    public ModelRepresentation importProcessModel(HttpServletRequest request, MultipartFile file) {
        ...
        if (fileName != null && (fileName.endsWith(".bpmn") || fileName.endsWith(".bpmn20.xml"))) {
            try {
                ...
                Model newModel = modelService.createModel(model, modelNode.toString(), null);
        ...
        if (fileName != null && (fileName.endsWith(".cmmn") || fileName.endsWith(".cmmn.xml"))) {
            try {
                ...
                if (CollectionUtils.isEmpty(cmmnModel.getCases())) {
                    throw new RuntimeException("No cases found in definition " + fileName);
                }

                if (cmmnModel.getLocationMap().size() == 0) {
                    throw new RuntimeException("No CMMN DI found in definition " + fileName);
                }
            ...
            throw new RuntimeException("Invalid file name, only .cmmn and .cmmn.xml files are supported not " + fileName);
        }
    }
